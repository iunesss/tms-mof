...
const {
  getAgentSector,
} = require('./agent.dashboard.controller');
...
async function decideNominations(req, res) {
  ...
  const nominationIds = Array.isArray(req.body.nominationIds)
    ? req.body.nominationIds.map(Number)
    : [];
  ...
  if (
    ...
    return sendError(res, 400, 'حدد مرشحًا واحدًا على الأقل.');
  }

  if (!isConfirmed && !reason) {
    return sendError(res, 400, 'سبب رفض الترشيحات مطلوب.');
  }
