...
function getCategory(notificationType = '') {
  ...
  if (type.includes('CANDIDATE') || type.includes('NOMINATION')) {
    return 'CANDIDATES';
  }
...
function getCategoryCondition(category, conditions) {
  ...
  if (category === 'CANDIDATES') {
    conditions.push(`
      (
        n.notification_type LIKE '%CANDIDATE%'
  ...
  if (category === 'GENERAL') {
    conditions.push(`
      n.notification_type NOT LIKE '%DOCUMENT%'
...
async function listNotifications(req, res) {
  try {
    ...
    const readStatus = String(req.query.readStatus || '').trim().toUpperCase();
    ...
    if (allowedCategories.includes(category)) {
      getCategoryCondition(category, conditions);
    }
    ...
    console.error('Course manager notifications error:', error);
    return sendError(res, 500, 'تعذر تحميل الإشعارات.');
  }
}
...
async function markNotificationAsRead(req, res) {
  try {
    ...
    return sendError(res, 500, 'تعذر تحديث الإشعار.');
  }
}
...
async function markAllNotificationsAsRead(req, res) {
  try {
    ...
    return sendError(res, 500, 'تعذر تحديث الإشعارات.');
  }
}
