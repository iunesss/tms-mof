...
async function getArchivedCourses(req, res) {
  try {
    // /api/courses/archive يشارك هذه العملية ولم يمر بvalidator التقارير بعد.
    const query = req.validatedQuery || req.query;
    const page = positiveNumber(query.page, 1);
    const limit = Math.min(positiveNumber(query.limit, 12), 100);
    const offset = (page - 1) * limit;

    const search = String(query.search || '').trim();
    const courseType = String(query.courseType || '').trim();
