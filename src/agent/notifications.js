...
function renderNotifications(notifications) {
              ...
              <div>
                <div class="flex flex-wrap items-center gap-2">
                  <span class="rounded-full border px-2.5 py-1 text-[10px] font-bold ${categoryClass(
                    ...
                    ${escapeHtml(categoryLabels[notification.category] || 'إشعار عام')}
                  </span>
  ...
  document.querySelectorAll('.notification-card').forEach((card) => {
    card.addEventListener('click', async () => {
      ...
      try {
        await api(`/api/agent/notifications/${notificationId}/read`, {
          method: 'PATCH',
        });
...
async function loadNotifications() {
  ...
  const data = await api(
    `/api/agent/notifications?${query.toString()}`
  );
...
async function markAllAsRead() {
  ...
  try {
    await api('/api/agent/notifications/read-all', {
      method: 'PATCH',
    });
