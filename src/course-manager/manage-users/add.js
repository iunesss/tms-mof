...
function renderOrganizationOptions() {
  setOptions(
    ...
    'اختر وكيل القطاع',
    (agent) => `${agent.full_name} — ${agent.sector_name || 'بدون قطاع'}`
  );
  ...
  setOptions(
    ...
    'اختر مدير القسم',
    (manager) =>
      `${manager.full_name} — ${manager.department_name || 'بدون قسم'}`
  );
}
...
async function createUser(event) {
  ...
  if (!allowedRoles.includes(roleCode)) {
    showMessage('يمكنك إنشاء وكيل قطاع أو مدير قسم أو موظف فقط.');
    return;
  }
